...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Сотрудник) Тогда
		Объект.Сотрудник = уп_СотрудникиПовторноеИспользование.ПолучитьСотрудникаПоПользователю(Объект.Ссылка);
	КонецЕсли;
	Если ЗначениеЗаполнено(Объект.ФизическоеЛицо) Тогда
		Элементы.ФизическоеЛицо.Вид = ВидПоляФормы.ПолеНадписи;
		Элементы.ФизическоеЛицо.Гиперссылка = Истина;
	КонецЕсли;
	Если ЗначениеЗаполнено(Объект.Сотрудник) Тогда
		Элементы.Сотрудник.Вид = ВидПоляФормы.ПолеНадписи;
		Элементы.Сотрудник.Гиперссылка = Истина;
	КонецЕсли;
	//-уп
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ВРег(ИмяСобытия) = ВРег("Запись_ФизическиеЛица") 
		И Параметр.Свойство("Владелец") 
		И Параметр.Владелец = ЭтаФорма Тогда
		
		Объект.ФизическоеЛицо = Источник;
		УстановитьВидимостьСотрудника();
	КонецЕсли;
...
Процедура ФизическоеЛицоСоздание(Элемент, СтандартнаяОбработка)
	...
	ПараметрыЗначенияЗаполнения.Вставить("Наименование", Объект.Наименование);
	
	ПараметрыФизЛица = Новый Структура();
	ПараметрыФизЛица.Вставить("ЗначенияЗаполнения", ПараметрыЗначенияЗаполнения);  	
	
	ОткрытьФорму("Справочник.ФизическиеЛица.Форма.ФормаЭлемента", ПараметрыФизЛица, ЭтаФорма);
КонецПроцедуры

&НаКлиенте
Процедура СотрудникСоздание(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	
	ПараметрыЗначенияЗаполнения = Новый Структура;
	ПараметрыЗначенияЗаполнения.Вставить("ФизическоеЛицо", Объект.ФизическоеЛицо);
	ПараметрыЗначенияЗаполнения.Вставить("Наименование", Объект.Наименование);
	
	ПараметрыСотрудника = Новый Структура();
	ПараметрыСотрудника.Вставить("ЗначенияЗаполнения", ПараметрыЗначенияЗаполнения);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ОбработкаСозданияСотрудника", ЭтотОбъект);
	ОткрытьФорму("Справочник.Сотрудники.Форма.ФормаЭлемента", ПараметрыСотрудника, ЭтаФорма, Истина, ,, ОписаниеОповещения);
КонецПроцедуры
...
//УП служебные процедуры

//Уп
//Продолжение процедуры СоздатьСотрудникаНажатие
&НаКлиенте
Процедура ОбработкаСозданияСотрудника(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	УстановитьВидимостьСотрудника();	
КонецПроцедуры

//Уп
&НаКлиенте
Процедура УстановитьВидимостьСотрудника()
	//Если ЗначениеЗаполнено(Объект.ФизическоеЛицо) Тогда
	//	Сотрудник = ПолучитьСотрудникаПоФизЛицу(Объект.ФизическоеЛицо);
	//	Если ЗначениеЗаполнено(Сотрудник) Тогда
	//		Элементы.Сотрудник.Видимость = Истина;
	//		Элементы.СоздатьСотрудника.Видимость = Ложь;
	//	Иначе
	//		Элементы.Сотрудник.Видимость = Ложь;
	//		Элементы.СоздатьСотрудника.Видимость = Истина;
	//	КонецЕсли;
	//Иначе
	//	Элементы.СоздатьСотрудника.Видимость = Ложь;
	//	Элементы.Сотрудник.Видимость = Ложь;
...
Функция ПолучитьСотрудникаПоФизЛицу(ФизЛицо)
	Возврат уп_СотрудникиПовторноеИспользование.ПолучитьСотрудникаПоФизЛицу(ФизЛицо);
КонецФункции
